...
public:
    ...
    binpack::Rect Insert(int width, int height, bool merge, FreeRectChoiceHeuristic rectChoice, GuillotineSplitHeuristic splitMethod);
...
//	void InsertMaxFitting(std::vector<RectSize> &rects, std::vector<binpack::Rect> &dst, bool merge, 
//		FreeRectChoiceHeuristic rectChoice, GuillotineSplitHeuristic splitMethod);
	...
	std::vector<binpack::Rect> &GetFreeRectangles() { return freeRectangles; }
	...
	std::vector<binpack::Rect> &GetUsedRectangles() { return usedRectangles; }
...
private:
	...
	binpack::Rect FindPositionForNewNode(int width, int height, FreeRectChoiceHeuristic rectChoice, int *nodeIndex);

	static int ScoreByHeuristic(int width, int height, const binpack::Rect &freeRect, FreeRectChoiceHeuristic rectChoice);
	...
	static int ScoreBestAreaFit(int width, int height, const binpack::Rect &freeRect);
	static int ScoreBestShortSideFit(int width, int height, const binpack::Rect &freeRect);
	static int ScoreBestLongSideFit(int width, int height, const binpack::Rect &freeRect);

	static int ScoreWorstAreaFit(int width, int height, const binpack::Rect &freeRect);
	static int ScoreWorstShortSideFit(int width, int height, const binpack::Rect &freeRect);
	static int ScoreWorstLongSideFit(int width, int height, const binpack::Rect &freeRect);
	...
	void SplitFreeRectByHeuristic(const binpack::Rect &freeRect, const binpack::Rect &placedRect, GuillotineSplitHeuristic method);

	/// Splits the given L-shaped free rectangle into two new free rectangles along the given fixed split axis.
	void SplitFreeRectAlongAxis(const binpack::Rect &freeRect, const binpack::Rect &placedRect, bool splitHorizontal);
};
